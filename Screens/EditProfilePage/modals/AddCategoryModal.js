...
const AddCategoryModal = ({ visible, onClose, type, pageType }) => {
    ...
    // console.log('services fetching for modal opening: ' , services);
    const handleSelectSub = (service, sub) => {
        // console.log('services fetching for modal sub: ' , sub);
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            if (result.status === 200) {
                ...
                const currentCategory = useEditProfileStore.getState().form.category;

                const newCategories = result.serviceMainData || [];
                console.log(newCategories);
                if (newCategories.length > 0) {
                    const merged = [
                        ...currentCategory,
                        ...newCategories.filter(
                            (nc) => !currentCategory.some((cc) => cc.subid === nc.subid)
                        ),
                    ];
                    useEditProfileStore.getState().setAllData({ category: merged });
                }
